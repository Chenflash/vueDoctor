  ...
  <div class="xurikangHomeland">
       ...
       <div v-if="!noFetch" v-show="!isloading">
        ...
        <ul class="listCenter"
              ...
              infinite-scroll-distance="0"  v-show="!noData">
                <li class="listItem" v-for="(list,i) of listHome" :key="i" @click="toHomeTitle(list.article_id)">
                 <p class="centersImg"> <img :src="list.article_pic" class="" :onerror="defaultImg"/></p>
                  <div class="rightItem">
                    <p class="rightTitle">{{list.article_title.length>40?list.article_title.slice(0,40)+"...":list.article_title}}</p>
                    <p class="centerCreated">{{list.article_time}}</p>
                    <p class="look_click">浏览量：{{list.article_click}}</p>
                  </div>
  ...
  methods:{
    ...
    fetchHome(){  //第一次加载
      var domain = process.env.NODE_ENV == 'development' ? '/api' : 'http://' + window.location.host;
       ...
       axios({
          method: 'POST',
           url: domain+'/mobile/index.php?act=article&op=article_home_list',
           data:{
    ...
    toHomeTitle(ids){
      ...
      window.location.href=domain+"#/articleDetail/"+ids+"?key="+this.$route.query.key;
    },
