<template>
  <div class="provideManSelf">
     <!--服务商角色，管理的服务商-->
     
         
         <div v-if="!noFetch"  v-show="!isloading">
            <div class="provideManSelfContainer">
              <ul class="integralCenter"  
               v-infinite-scroll="loadMore" 
               infinite-scroll-disabled="loading"
              infinite-scroll-immediate-check="false"
               v-if="!noData">
                <li class="listItem" v-for="(item,i) of provideManSelfList" :key="i" @click="toDetail(item.member_id)">
       ...
       var domain = process.env.NODE_ENV == 'development' ? '/api' : 'http://' + window.location.host;
        var that=this;
          setTimeout(()=>{
              axios({
                    method: 'Post',
                    url:domain+'/mobile/index.php?act=member_index&op=my_own_facilitator',
                    data:{
              ...
              that.$store.commit("decrement");
               ...
               if(that.isCount==0){
                 ...
                 that.provideManSelfList=results;
                 that.isMore=true;
                that.showMore=true; //没有更多
                 that.haveMore = false;   
                  ...
                  that.noData=true;
                   ...
                   that.provideManSelfList=that.provideManSelfList.concat(results);
