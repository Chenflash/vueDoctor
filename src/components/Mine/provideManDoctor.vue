<template>
  <div class="provideManDoctor">
     <!--服务商角色，管理的医生-->
     
         
         <div v-if="!noFetch"  v-show="!isloading">
            <div class="provideManDoctorContainer">
              <ul class="integralCenter"  
               v-infinite-scroll="loadMore" 
               infinite-scroll-disabled="loading"
              infinite-scroll-immediate-check="false"
               v-if="!noData">
                <li class="listItem" v-for="(item,i) of provideManDoctorList" :key="i" @click="toDetail(item.member_id)">
       ...
       var domain = process.env.NODE_ENV == 'development' ? '/api' : 'http://' + window.location.host;
        var that=this;
          setTimeout(()=>{
              axios({
                    method: 'Post',
                    url:domain+'/mobile/index.php?act=member_index&op=my_own_doctor',
                    data:{
              ...
              that.$store.commit("decrement");
               ...
               if(that.isCount==0){
                 ...
                 that.provideManDoctorList=results;
                 that.isMore=false;
                 that.haveMore = false;   
                  ...
                  that.noData=true;
                   ...
                   that.provideManDoctorList=that.provideManDoctorList.concat(results);
