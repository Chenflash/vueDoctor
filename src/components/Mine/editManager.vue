<template>
  <div class="editManager">
        ...
        <div class="editManagerContainer">
           <textarea cols="40" rows="10" class="inputTextarea"  v-model="msg"/>   
           <input type="submit" value="保存" class="btnSubmit" :disabled="isDisable"  @click="submiteditManager"/>
...
export default {
  ...
  created() {
       var domain = process.env.NODE_ENV == 'development' ? '/api' : 'http://' + window.location.host;
            ...
            axios({
                    method: 'Post',
                    url:domain+'/mobile/index.php?act=member_account&op=store_speciality',
                    data:{
    ...
    submiteditManager(){   //提交
       if(this.msg!=""){
         ...
         var domain = process.env.NODE_ENV == 'development' ? '/api' : 'http://' + window.location.host;
            ...
            axios({
                    method: 'Post',
                    url:domain+'/mobile/index.php?act=member_account&op=store_speciality_edit',
                    data:{
                       key:that.$route.query.key,
                       msg:that.msg,
                       save_type:2,
