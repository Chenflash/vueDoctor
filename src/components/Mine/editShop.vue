<template>
  <div class="editShop">
        ...
        <div class="editShopContainer">
           <textarea cols="40" rows="10" class="inputTextarea"  v-model="msg"/>   
           <input type="submit" value="保存" class="btnSubmit" :disabled="isDisable"  @click="submiteditShop"/>
...
export default {
  ...
  created() {
       var domain = process.env.NODE_ENV == 'development' ? '/api' : 'http://' + window.location.host;
            ...
            axios({
                    method: 'Post',
                    url:domain+'/mobile/index.php?act=member_account&op=store_desc',
                    data:{
    ...
    submiteditShop(){   //提交
       if(this.msg!=""){
         ...
         var domain = process.env.NODE_ENV == 'development' ? '/api' : 'http://' + window.location.host;
            ...
            axios({
                    method: 'Post',
                    url:domain+'/mobile/index.php?act=member_account&op=store_desc_edit',
                    data:{
                       key:that.$route.query.key,
                       msg:that.msg,
                       save_type:1, 
